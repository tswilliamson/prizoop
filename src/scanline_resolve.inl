...
inline unsigned int GetPaletteForSrc(int srcValue) {
	DebugAssert((srcValue & 3) == 0);
	return ppuPalette[srcValue >> 2];
}
...
#else
		*(scanline++) = (GetPaletteForSrc(lineBuffer[i]) << 16) | ((unsigned short)GetPaletteForSrc(lineBuffer[i + 1]));
#endif
	}
}
...
#ifdef LITTLE_E
		const unsigned short color1 = (unsigned short)GetPaletteForSrc(src[1]);
		const unsigned short color2 = (unsigned short)GetPaletteForSrc(src[2]);
		const unsigned int color3 = GetPaletteForSrc(src[3]); // no cast needed since bit shifted

		dest[1] = (color2 << 16) | (color1);
		dest[2] = (color3 << 16) | color2;
#else
		const unsigned int color1 = GetPaletteForSrc(src[1]); // no cast needed since bit shifted
		const unsigned short color2 = (unsigned short)GetPaletteForSrc(src[2]);
		const unsigned short color3 = (unsigned short)GetPaletteForSrc(src[3]);
...
inline void BlendScanline24(unsigned int* dest) {
	...
	for (int i = 0; i < 40; i++, dest += 3, src += 4) {
		...
		unsigned short color2 = (unsigned short) GetPaletteForSrc(src[2]);
		unsigned short color3 = (unsigned short) GetPaletteForSrc(src[3]);
...
inline void BlendMixedScanline24(unsigned int* dest) {
	...
	for (int i = 0; i < 40; i++, dest += 3, src += 4, src2 += 4) {
		...
		unsigned short color2 = mix565((unsigned short) GetPaletteForSrc(src[2]), (unsigned short) GetPaletteForSrc(src2[2]));
		unsigned short color3 = mix565((unsigned short) GetPaletteForSrc(src[3]), (unsigned short) GetPaletteForSrc(src2[3]));
...
inline void BlendTripleScanline24(unsigned int* scanline, int*src0, int* src1, unsigned int* ppuPalette) {
	...
	for (int i = 0; i < 40; i++, scanline1 += 3, scanline2 += 3, scanline3 += 3, src0 += 4, src1 += 4) {
		const unsigned short color00 = (unsigned short) GetPaletteForSrc(src0[0]);
		const unsigned short color01 = (unsigned short) GetPaletteForSrc(src0[1]);
		const unsigned short color02 = (unsigned short) GetPaletteForSrc(src0[2]);
		const unsigned short color03 = (unsigned short) GetPaletteForSrc(src0[3]);
		const unsigned short color10 = (unsigned short) GetPaletteForSrc(src1[0]);
		const unsigned short color11 = (unsigned short) GetPaletteForSrc(src1[1]);
		const unsigned short color12 = (unsigned short) GetPaletteForSrc(src1[2]);
		const unsigned short color13 = (unsigned short) GetPaletteForSrc(src1[3]);
...
inline void DirectTripleScanline24(unsigned int* scanline1, unsigned int* scanline2, unsigned int* scanline3) {
	...
	for (int i = 0; i < 40; i++, scanline1 += 3, scanline2 += 3, scanline3 += 3, src0 += 4, src1 += 4) {
		...
		const unsigned int color10 = GetPaletteForSrc(src1[0]);
		scanline2[0] = color10;
		scanline3[0] = color10;
		...
		const unsigned short color01 = (unsigned short)GetPaletteForSrc(src0[1]);
		const unsigned short color02 = (unsigned short)GetPaletteForSrc(src0[2]);
		const unsigned int   color03 = GetPaletteForSrc(src0[3]); // no cast needed since bit shifted
		const unsigned short color11 = (unsigned short)GetPaletteForSrc(src1[1]);
		const unsigned short color12 = (unsigned short)GetPaletteForSrc(src1[2]);
		const unsigned int   color13 = GetPaletteForSrc(src1[3]); // no cast needed since bit shifted
		...
		scanline3[2] = dest1b;
#else
		const unsigned int   color01 = GetPaletteForSrc(src0[1]); // no cast needed since bit shifted
		const unsigned short color02 = (unsigned short)GetPaletteForSrc(src0[2]);
		const unsigned short color03 = (unsigned short)GetPaletteForSrc(src0[3]);
		const unsigned int   color11 = GetPaletteForSrc(src1[1]); // no cast needed since bit shifted
		const unsigned short color12 = (unsigned short)GetPaletteForSrc(src1[2]);
		const unsigned short color13 = (unsigned short)GetPaletteForSrc(src1[3]);
