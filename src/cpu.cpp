...
static inline void adc(unsigned char value) {
	...
	if(((value & 0x0f) + (cpu.registers.a & 0x0f)) > 0x0f) FLAGS_SET(FLAGS_HALFCARRY);
	else FLAGS_CLEAR(FLAGS_HALFCARRY);
...
static inline void sbc(unsigned char value) {
	...
	if(value == cpu.registers.a) FLAGS_SET(FLAGS_ZERO);
	else FLAGS_CLEAR(FLAGS_ZERO);
...

	{
		...
		for (int i = 0; i < CPU_BATCH; i++) {
#if DEBUG
			// General breakpoints
			//if(cpu.registers.pc == 0x034c) { // incorrect load
			//if(cpu.registers.pc == 0x0309) { // start of function which writes to ff80
			//if(cpu.registers.pc == 0x2a02) { // closer to function call which writes to ff80
			//if(cpu.registers.pc == 0x034c) { // function which writes to ffa6 timer

			//if(cpu.registers.pc == 0x036c) { // loop
			//if(cpu.registers.pc == 0x0040) { // vblank

			//if(cpu.registers.pc == 0x29fa) { // input
			...
			switch (readByte(cpu.registers.pc++)) {
				...
				#define INSTRUCTION_1(name,numticks,func,id,code)   case id: DebugInstruction(name); { unsigned char operand = readByte(cpu.registers.pc++); func(operand); cpu.ticks += numticks; code } break;
				#define INSTRUCTION_1S(name,numticks,func,id,code)  case id: DebugInstruction(name); { signed char operand = readByte(cpu.registers.pc++); func(operand); cpu.ticks += numticks; code } break;
				#define INSTRUCTION_2(name,numticks,func,id,code)   case id: DebugInstruction(name); { unsigned short operand = readShort(cpu.registers.pc++); ++cpu.registers.pc; func(operand); cpu.ticks += numticks; code } break;
